...
3. **No third-party network calls.** Only `cdn.vocadesk.com`/`embed-cdn.callcat.ai` (script load), `embed.vocadesk.com`/`embed.callcat.ai` (tokens), `voice.vocadesk.com`/`voice.callcat.ai` (WSS), and `challenges.cloudflare.com` when Turnstile is on. No analytics, no Sentry, no fonts, no CDN images. This is a security promise to our customers. The `vocadesk.com` domains are permanent aliases for already-deployed customer embeds (pinned to old bundle versions) — new embeds (v0.6.0+) default to the `callcat.ai` ones instead. Any customer-facing docs/CSP guidance must list whichever pair matches the SDK version they're actually using.
